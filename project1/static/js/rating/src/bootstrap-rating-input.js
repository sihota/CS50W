(function ($) {
  ...
  var clearClass = 'rating-clear',
    ...
    DEFAULTS = {
      ...
      'max': 5,
      'empty-value': 0,
      'iconLib': 'fa',//'glyphicon',
      'activeIcon': 'fa-star',//'glyphicon-star',
      'inactiveIcon': 'fa-star-o', //'glyphicon-star-empty',
      'clearable': false,
      'clearableIcon': 'fa-remove',//'glyphicon-remove',
  ...
  function parseOptions($input, options) {
    ...
    if (data.clearableicon === "no") {
      data.clearableiconLabel = '';
    } else {      
      data.clearableiconLabel = DEFAULTS.clearableIcon
    }
    data.clearableicon = data.clearableicon === '' || data.clearableicon;
    return data;
  }
  ...
  function createRatingEl($input, options) {
    ...
    if (options.clearable && !options.readonly) {
      $ratingEl.append('&nbsp;').append(
        '<a class="' + clearClass + '">' +
          '<i class="' + options.iconLib + ' ' + options.clearableiconLabel + '"/>' +
